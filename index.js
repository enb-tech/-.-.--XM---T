...
setTimeout(() => {
    async function main() {
        ...
        const sockOptions = {
            ...
            browser: ['Σ.И.ß-XM𝐃-𝚩𝚯T', "safari", "1.0.0"],
...
zk.ev.on("messages.upsert", async (m) => {
  ...
  if (messageType === "conversation" || messageType === "extendedTextMessage") {
    ...
    const systemMessage = { role: 'system', content: 'You are called Σ.И.ß-XM𝐃-𝚩𝚯T. Developed by UTYATYA GRAYSON. You respond to user commands. Only mention developer name if someone asks.' };
...
setInterval(async () => {
    if (conf.AUTO_BIO === "yes") {
        ...
        const bioText = `Σ.И.ß-XM𝐃-𝚩𝚯T is online! 🚀\n${currentDateTime}`; // Format the bio text
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'on')) {
            let msg = `*Σ.И.ß-XM𝐃-𝚩𝚯T WELCOME MESSAGE*`;
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ Σ.И.ß-XM𝐃-𝚩𝚯T is connecting...");
            }
            else if (connection === 'open') {
                console.log("✅ Σ.И.ß-XM𝐃-𝚩𝚯T Connected to WhatsApp! ☺️");
                console.log("--");
                await (0, baileys_1.delay)(200);
                console.log("------");
                await (0, baileys_1.delay)(300);
                console.log("------------------/-----");
                console.log("Σ.И.ß-XM𝐃-𝚩𝚯T is Online 🕸\n\n");
                //chargement des commandes 
                console.log("Loading Σ.И.ß-XM𝐃-𝚩𝚯T Commands ...\n");
                fs.readdirSync(__dirname + "/Σ.И.ß-XM𝐃-𝚩𝚯T").forEach((fichier) => {
                    if (path.extname(fichier).toLowerCase() == (".js")) {
                        try {
                            require(__dirname + "/Σ.И.ß-XM𝐃-𝚩𝚯T/" + fichier);
